...
longer_delay = 250 # '' if standard delay (150), longer can be 200, 250, 400
...
results_dir = f'{models_dir}models_good+bad/' # combined results dir
# np.save(f'{results_dir}models_bandmeans{norm_name[0]}{lesion_name[0]}{delay_name}.npy', models_bandmeans)
...
for i, power in enumerate(freqbands_list):
    ...
    plt.legend()
    plt.savefig(f'{results_dir}{power}_power_diff_good+bad{norm_name[0]}{lesion_name[0]}{delay_name}.png')




# # compare good vs bad models (subtraction)
# for i, power in enumerate(freqbands_list):
#     print(f'Plotting {power} power difference...')
#     if isinstance(power, str):
#         if power == "theta":
#             power_range = [4,8]
#         elif power == "alpha":
...
#     for j, neuron_type in enumerate(['excitatory','inhibitory']):
#         for k, trial_type in enumerate(['+1', '-1']):
#             band_diff = models_bandmeans[0][i,j,k,:] - models_bandmeans[1][i,j,k,:] # good - bad
#             plt.plot(band_diff, label=f'{trial_type} stim1, {neuron_type}', c=['r','b'][k], alpha=1, linestyle=['-','--'][j])
...
#     else:
#         lesion_name_title = ''
#     plt.title(f'Difference in {power} ({power_range[0]}-{power_range[1]}Hz) power in (good - bad) models {norm_name[0]}{lesion_name_title}')
#     plt.legend()
    
#     plt.savefig(f'{results_dir}{power}_power_diff_good-bad{norm_name[0]}{lesion_name[0]}.png')
