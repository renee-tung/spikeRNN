# Script to make ERPAC plots across models, comparing good models with lesioned models
...
all_mean_erpacs = [] # len = n_lesion_connections
all_mean_diff_erpacs = []
all_delay_erpacs = []

for lesion_connection in lesion_connections_list:
    lesion_name = ['' if lesion_connection == 0 else f'_lesion{lesion_connection}']
...
# plot difference between good and lesioned models; and then difference between +1/-1 trials between good [0] and lesioned [1] models
for i, neuron_label in enumerate(neuron_labels):
    for j, trial_label in enumerate(trial_labels):
        vu.plot_erpac(rp_obj, all_mean_erpacs[0][i*len(neuron_labels) + j] - all_mean_erpacs[1][i*len(neuron_labels) + j], times, freqs, fs_spk, settings, settings_real=1,
                title = f'Mean ERPAC difference, {neuron_label}, {trial_label}, good minus {lesion_connections_list[1]} lesioned',
                vmin=-.15, vmax=.15, cmap='bwr',
                save=1, savename = f'{results_dir}erpac_goodlesiondiff_{trial_label}{neuron_label}{norm_name[0]}{lesion_name[0]}{delay_name}.png')
    vu.plot_erpac(rp_obj, all_mean_diff_erpacs[0][i] - all_mean_diff_erpacs[1][i], times, freqs, fs_spk, settings, settings_real=1,
                title = f'Mean ERPAC difference, {neuron_label}, +1 minus -1, then good minus {lesion_connections_list[1]} lesioned',
                vmin=-.25, vmax=.25, cmap='bwr',
                save=1, savename = f'{results_dir}erpac_goodlesiondiff_trialdiff_{neuron_label}{norm_name[0]}{lesion_name[0]}{delay_name}.png')
...
for i, neuron_label in enumerate(neuron_labels):
    for j, trial_label in enumerate(trial_labels):
        ...
        plt.plot(freqs, bad_erpac_delay, label=f'{lesion_connections_list[1]} lesion', color='b')
...
for i, neuron_label in enumerate(neuron_labels):
    for j, trial_label in enumerate(trial_labels):
        ...
        ax.plot(freqs, bad_erpac_delay, label=f'{lesion_connections_list[1]} lesion {neuron_label} {trial_label}', color=colors[this_idx], linestyle='--')
...
for i, neuron_label in enumerate(neuron_labels):
    ...
    ax.plot(freqs, bad_erpac_delay, label=f'{lesion_connections_list[1]} lesion {neuron_label}', color='b')
