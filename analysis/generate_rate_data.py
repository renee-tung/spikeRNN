...
def get_all_stim_combos() -> np.ndarray:
    ...
    values = [0, 1, 2, 3]
    first_three = list(permutations(values, 3))
    full = [(a, b, c, d) for (a, b, c) in first_three for d in values]
    perm_part = np.array(full, dtype=np.float32)

    inf_val = np.inf
    middle_inf_part = np.array(
        [[x, inf_val, inf_val, y] for x, y in product(values, repeat=2)],
        dtype=np.float32,
    )
    return np.vstack([perm_part, middle_inf_part]).astype(np.float32)
