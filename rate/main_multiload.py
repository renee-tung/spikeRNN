...
from model import generate_target_continuous_sternberg
...
parser.add_argument("--task_load", required=True,
                    nargs='+',type=float, 
                    help="Load(s) for Sternberg task. If only one number is given, then all trials will have that load. \
                        Otherwise specify multiple numbers and the load will alternate between those values during training \
                            in equal proportions.")
parser.add_argument("--eval_thresholds", required=True,
                    nargs='+', type=float,
                    help="Evaluation thresholds for each load. eg 0.95, 0.9, 0.8 for load 1, 2, and 3, respectively.")
...
if args.task.lower() == 'go-nogo':
    ...
    settings = {
            ...
            'taus': args.decay_taus, # decay time-constants (in steps)
            'task': args.task.lower(), # task name
            'load': task_loads[0], # initialize with first load, but will alternate with others during training
            }
    def adjust_T_sternberg(T, stim_on, stim_dur, delay, load_max):
        ...
        T = int(np.amax((T, min_T)))
        return T
    load_max = np.amax(task_loads)
    settings['T'] = adjust_T_sternberg(settings['T'], settings['stim_on'], 
                                       settings['stim_dur'], settings['delay'], load_max=load_max)
...
training_params = {
        ...
        'eval_tr': 75, # number of trials for eval -- decreased since more loads to evaluate on
...
if args.mode.lower() == 'train':
    with tf.Session() as sess:
        ...
        if args.task.lower() == 'go-nogo':
            ...
            # p_low = load_proportion # probability of low-load trials during training; set to 0.5 for 50/50 alternating
            settings['load'] = 1 # start with load 1; np.random.choice([1, 3], p=[p_low, 1-p_low]) 
        ...
        eval_perfs_all = [] # for storing perf for both loads on Sternberg task

        for tr in range(args.n_trials): # loop thru training trials
            ...
            if args.task.lower() == 'go-nogo':
                ...
                settings['load'] = np.random.choice(task_loads) # choose load by equal probability
                # add time jitter to stimulus onset and delay period
                if args.jitter_onset > 0 or args.jitter_delay > 0:
                    settings_jitter = settings.copy()
                    settings_jitter['stim_on'] = settings['stim_on'] + np.random.randint(-args.jitter_onset, args.jitter_onset+1)
                    ...
                    u, label = generate_input_stim_sternberg(settings)
                    target = generate_target_continuous_sternberg(settings, label)
            print("Trial " + str(tr) + ': ' + str(label) + " load: " + str(settings['load']))
            ...
            # Sternberg working memory task
                ...
                if (tr-1)%training_params['eval_freq'] == 0:
                    eval_perfs = np.zeros(len(task_loads)) # for storing perf for each load
                    for j, load in enumerate(task_loads): # eval_tr number of trials for each load
                        settings['load'] = load
                        eval_perf = np.zeros((1, training_params['eval_tr']))
                        eval_losses = np.zeros((1, training_params['eval_tr']))
                        eval_os = np.zeros((training_params['eval_tr'], settings['T']-1))
                    ...
                    eval_perfs_all.append(eval_perfs)
                    
                    # if eval_loss_mean < training_params['loss_threshold'] and eval_perf_mean > 0.95:
                    #     training_success = True
                    #     break
                    if all(eval_perfs[i] > args.eval_thresholds[i] for i in range(len(task_loads))):
                        training_success = True
                        break
        ...
        if args.task.lower() == 'sternberg':
            var['task_loads'] = task_loads
            var['eval_thresholds'] = args.eval_thresholds
            var['eval_perfs'] = eval_perfs
            var['eval_perfs_all'] = np.array(eval_perfs_all)
