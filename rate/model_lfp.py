...
class FR_RNN_dale:
    ...
    """
    def __init__(self, N, P_inh, P_rec, w_in_stim, som_N, w_dist, gain, apply_dale, w_out):
        """
...
def generate_input_stim_xor(settings):
    ...
    OUTPUT
        ...
        label: '+1' or '-1' for the first stimulus
    """
    T = settings['T']
    ...
    label = labs[0] # stim 1 identity — +1 or -``
...
'''
def generate_LFP_target_xor(settings, u):
    """
    ...
    OUTPUT
        lfp_target: list of length T, each element is a [1,1] tensor
    """
    T = settings['T']
    stim_on = settings['stim_on']
    stim_dur = settings['stim_dur']
    ...
    lfp_target = np.zeros((1, T), dtype=np.float32)
    stim1 = u[0, stim_on+1]
    ...
    if len(target_freq) > 1:
        for f in target_freq:
            ...
            lfp_target[0, period[0]:period[1]] += wave / len(target_freq)
    else:
        f = target_freq[0]
        wave = np.sin(2 * np.pi * f / fs * time * DeltaT)
        lfp_target[0, period[0]:period[1]] = wave

    lfp_target = lfp_target * stim1 # different phases for +1 or -1 first stim

    return np.squeeze(lfp_target)
...
def construct_tf(fr_rnn, settings, training_params):
    ...
    for t in range(1, T):
        ...
        next_x = tf.multiply((1 - DeltaT/taus_sig), x[t-1]) + \
                tf.multiply((DeltaT/taus_sig), ((tf.matmul(ww, r[t-1]))\
                + tf.matmul(w_in_stim, tf.expand_dims(stim[:, t-1], 1)))) +\
                tf.random_normal([fr_rnn.N, 1], dtype=tf.float32)/10
        x.append(next_x)
    ...
    return stim, z, y, x, r, epsp, o, w, w_in_stim, m, som_m, w_out, b_out, taus_gaus
...
def eval_tf(model_dir, settings, u, lesion='', lesion_perc=0.5, calc_epsp=True):
    ...
    if len(lesion) != 0:
        ...
        #         w_in_lfp[inh_ind, 0] = w_in_lfp[inh_ind, 0] * lesion_perc
            ...
            raise ValueError("Invalid lesion type. Choose either 'ii', 'ei', 'ie', or 'ee'.")
    ...
    for t in range(1, T):
        ...
        next_x = np.multiply((1 - DeltaT/taus_sig), np.expand_dims(x[:, t-1], 1)) + \
                np.multiply((DeltaT/taus_sig), ((np.matmul(ww, np.expand_dims(r[:, t-1], 1)))\
                + np.matmul(w_in_stim, np.expand_dims(u[:, t-1], 1)))) +\
                np.random.randn(N, 1)/10
