# Script to make PAC plots across models, comparing good models with lesioned models
...
all_mean_pacs = [] # len = n_lesion_connections
all_mean_diff_pacs = []
all_delay_pacs = []
...
for lesion_connection in lesion_connections_list:
    ...
    for n_model, model_fname in enumerate(model_list): # for each individual model
        ...
        # calculate mean pac across models, mean difference btwn +1/-1 trials
    ...
    # store mean delay pacs across models of this model type
    all_delay_pacs.append(np.array(delay_pacs)) # appending a n_models x 4 x n_freqs array for each model type
...
for i, neuron_label in enumerate(neuron_labels):
    for j, trial_label in enumerate(trial_labels):
        vu.plot_pac(pac_obj, all_mean_pacs[0][i*len(neuron_labels) + j] - all_mean_pacs[1][i*len(neuron_labels) + j],
                title = f'Mean PAC difference, {neuron_label}, {trial_label}, good minus {lesion_connections_list[1]} lesioned',
                vmin=-.002, vmax=.002, cmap='bwr',
                save=1, savename = f'{results_dir}pac_goodlesiondiff_{trial_label}{neuron_label}{norm_name[0]}{lesion_name[0]}{delay_name}.png')
    vu.plot_pac(pac_obj, all_mean_diff_pacs[0][i] - all_mean_diff_pacs[1][i], 
                title = f'Mean PAC difference, {neuron_label}, +1 minus -1, then good minus {lesion_connections_list[1]} lesioned',
                vmin=-.002, vmax=.002, cmap='bwr',
                save=1, savename = f'{results_dir}pac_goodlesiondiff_trialdiff_{neuron_label}{norm_name[0]}{lesion_name[0]}{delay_name}.png')
