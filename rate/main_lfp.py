...
from model_lfp import FR_RNN_dale

# Import the tasks from model_lfp
from model_lfp import generate_input_stim_xor
# from model_lfp import generate_target_continuous_xor
from model_lfp import generate_LFP_target_xor

# from model_lfp import generate_LFP_input_phase

from model_lfp import construct_tf
from model_lfp import loss_op
...
parser.add_argument("--target_power", required=True,
        nargs='+', type=float,
        help="Target power for LFP input. If only one number is given, then sine frequency is set\
            to be generated at this frequency only. Can also specify two numbers for a band range (min, max).")
parser.add_argument("--power_period", required=False,
        type=str, default='delay', help="Period during which to generate target power (either 'delay' or 'full')")
...
if args.task.lower() == 'go-nogo':
    ...
    settings = {
            ...
            'delay': 100, # delay b/w the two stimuli (in steps)
            'DeltaT': 1, # sampling rate
...
# XOR task
    ...
    w_in_stim = np.float32(np.random.randn(N, 2))
    # w_in_lfp = np.float32(np.random.randn(N, 1))
    w_out = np.float32(np.random.randn(1, N)/100)
...
net = FR_RNN_dale(N, P_inh, P_rec, w_in_stim, som_N, w_dist, args.gain, args.apply_dale, w_out)
...
training_params = { # increased loss threshold for LFP models
        ...
        'loss_threshold': 1, # loss threshold (when to stop training)
        # 'loss_threshold': (4/settings['lfp_power_target'][0])*(settings['delay']/200*settings['lfp_power_target'][0]), # loss threshold (when to stop training)
...
if args.mode.lower() == 'train':
    input_node, z, y, x, r, epsp, o, w, w_in_stim, m, som_m, w_out, b_out, taus\
...
if args.mode.lower() == 'train':
    with tf.Session() as sess:
        ...
        if args.task.lower() == 'go-nogo':
            ...
            target = generate_LFP_target_xor(settings, u)
            # target = generate_target_continuous_xor(settings, label)
            # u = generate_LFP_input_phase(settings, u)
            x0, r0, w0, w_in_stim0, taus_gaus0 = \
                    sess.run([x, r, w, w_in_stim, taus], feed_dict={input_node: u, z: target})
        ...
        for tr in range(args.n_trials):
            ...
            if args.task.lower() == 'go-nogo':
                ...
                if args.jitter_onset > 0 or args.jitter_delay > 0:
                    ...
                    target = generate_LFP_target_xor(settings_jitter, u)
            ...
            _, t_loss, t_w, t_o, t_w_out, t_x, t_r, t_m, t_som_m, t_w_in_stim, t_b_out, t_taus_gaus = \
                    sess.run([training_op, loss, w, o, w_out, x, r, m, som_m, w_in_stim, b_out, taus],
            ...
            # XOR task
                ...
                if (tr-1)%training_params['eval_freq'] == 0:
                    ...
                    eval_os = np.zeros((training_params['eval_tr'], settings['T']-1))
                    eval_labels = []
                    for ii in range(eval_losses.shape[-1]):
                        eval_u, eval_label = generate_input_stim_xor(settings)
                        eval_target = generate_LFP_target_xor(settings, eval_u)
                    ...
                    # eval_perf_mean = np.nanmean(eval_perf, 1)
                    eval_loss_mean = np.nanmean(eval_losses, 1)
                    print("Loss: %.2f"% (eval_loss_mean))

                    if eval_loss_mean < training_params['loss_threshold']:
                        training_success = True
                        break
        ...
        var['w_in_stim0'] = w_in_stim0
        # var['w_in_lfp0'] = w_in_lfp0
        var['u'] = u
        var['o'] = t_o
        var['w'] = t_w
        var['x'] = t_x
        ...
        var['exc'] = net.exc
        var['inh'] = net.inh
        var['w_in_stim'] = t_w_in_stim
        # var['w_in_lfp'] = t_w_in_lfp
        var['b_out'] = t_b_out
        var['som_N'] = som_N
        var['losses'] = losses
        ...
        #             training_params['activation'], fname_time)
        fname = 'Task_{}_N_{}_Jitter_{}_{}_LFP{}_{}_{}.mat'.format(args.task.lower(), N, args.jitter_onset, args.jitter_delay,
                    settings['power_target_period'], settings['lfp_power_target'][0], fname_time)
