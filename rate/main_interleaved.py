...
parser.add_argument("--load_proportion", required=False,
                    type=restricted_float, default=0.5, help="Proportion of low-load trials for Sternberg task")
...
out_dir = os.path.join(args.output_dir, args.task.lower(), 'interleaved_' + str(args.load_proportion))
...
if args.task.lower() == 'go-nogo':
    ...
    load_proportion = args.load_proportion
    if load_proportion > 1 or load_proportion < 0:
        raise ValueError('Invalid load for Sternberg task. Must be between 0 and 1.')
    settings = {
        ...
        for tr in range(args.n_trials): # loop thru training trials
            ...
            # Sternberg working memory task
                ...
                if (tr-1)%training_params['eval_freq'] == 0:
                    ...
                    ''' 
                    for this version, just keep p_low as load_proportion (eg 0.5) the whole time.
                    '''
