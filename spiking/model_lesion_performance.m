...
for n_type = 1:length(models_types) % good and bad models
    ...
    for n_model = 1:length(stable_mods) % for each model
        ...
        eval_perf_plus = zeros(length(lesion_connections), n_trials); % Store models' performance
...
% Example data for two categories (1 and 2), each with two groups
Group1_1 = randn(10,1) + 1;  % Data for Group1 at Category 1
Group2_1 = randn(10,1) + 1.5; % Data for Group2 at Category 1

Group1_2 = randn(10,1) + 2;  % Data for Group1 at Category 2
Group2_2 = randn(10,1) + 2.5; % Data for Group2 at Category 2

% Combine the data into one matrix (2 groups for each x-value)
data = [Group1_1, Group2_1, Group1_2, Group2_2];

% Create x labels (1 for Category 1 and 2 for Category 2)
x_labels = [ones(length(Group1_1),1);  % Category 1
            2*ones(length(Group1_2),1)]; % Category 2

% Create boxplot with two groups per x value
figure;
boxplot(data, x_labels);

% Set axis labels and title
xlabel('Category');
ylabel('Values');
title('Boxplot with Two Groups per X Value');