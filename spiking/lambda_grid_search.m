...
for i = 1:length(mat_files)
  ...
  % Sternberg task 
  elseif strcmpi(task_name, 'sternberg')
    down_sample = 1;
    loads = [1, 3];
    % loads = [1];
    ...
    for k = 1:length(scaling_factors)
      ...
      for i_load = 1:length(loads)
          ...
          all_perfs(k, i_load) = mean(perfs);
      
      end % load loop end
     
    end % scaling end
    [v_low, ind_low] = max(all_perfs(:,1));
    [v_high, ind_high] = max(all_perfs(:,2));
    [v_low, scaling_factors(ind_low)]
    [v_high, scaling_factors(ind_high)]

    all_perfs_mean = mean(all_perfs, 2);
    [v, ind] = max(all_perfs_mean);
    [v, scaling_factors(ind)]
