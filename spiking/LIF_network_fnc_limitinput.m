...
function [W, REC, spk, rs, all_fr, out, params] = LIF_network_fnc_limitinput(model_path,...
scaling_factor, u, stims, lfp, downsample, use_initial_weights, input_to)
...
% limit inputs
if ischar(input_to)
    if strcmp(input_to, 'exc')
        w_in_lfp(inh_ind) = 0;
    elseif strcmp(input_to, 'inh')
        w_in_lfp(exc_ind) = 0;
    end
else
    disp('limit_to is not a string, but should be exc or inh. limit_to=exc means LFP input only goes to exc neurons')
end
...
out = smoothdata(squeeze(out), "gaussian", 1000); % adding smoothing
