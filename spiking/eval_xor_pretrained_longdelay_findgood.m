...
% cycle through good models and plot 100 random trials (never ran this on
% all, not necessary)
for k = 1:length(stable_mods)
    model_name = stable_mods{k};
    ...
    % evaluate model on 100 random trials with longer delay period
    ...
    % for good models
    if eval_perf_mean > perf_threshold
        fnames = [fnames, model_name];
        disp([model_name(end-20:end-4) 'Performance: ' num2str(eval_perf_mean,3)])
...
stim_dur = 50;
delay = 150; % longer delay
stim2_offset = (stim_on + 2*stim_dur + delay)/200*20000;
...
eval_perf_mean = mean(eval_perf);
disp(['Performance: ' num2str(eval_perf_mean,3)])
title(['Performance: ' num2str(eval_perf_mean,3)])
saveas(gcf,[model_dir_path '/output_delay150.png'])
...
for i=1:n_trials
    [~, ~, ~, rs, ~, ~, ~] = LIF_network_fnc(file_path, scaling_factor,...
        u, stims, down_sample, use_initial_weights);
    pcs = pca(rs);
    pcs_samepos = pcs_samepos + pcs(:,1:3);
    clear pcs
end
pcs_samepos = pcs_samepos / n_trials;

% +1/-1
pcs_diffpos = zeros(T*100, 3);
u = zeros(2, T+1); % input stim
u(1, stim_on:stim_on+stim_dur) = 1; % first stim is +1
u(2, stim_on+stim_dur+delay:stim_on+2*stim_dur+delay) = -1; % second stim is -1
for i=1:n_trials
    [~, ~, ~, rs, ~, ~, ~] = LIF_network_fnc(file_path, scaling_factor,...
        u, stims, down_sample, use_initial_weights);
    pcs = pca(rs);
    pcs_diffpos = pcs_diffpos + pcs(:,1:3);
    clear pcs
end
pcs_diffpos = pcs_diffpos / n_trials;

% -1/-1
pcs_sameneg = zeros(T*100, 3);
u = zeros(2, T+1); % input stim
u(1, stim_on:stim_on+stim_dur) = -1; % first stim is -1
u(2, stim_on+stim_dur+delay:stim_on+2*stim_dur+delay) = -1; % second stim is -1
for i=1:n_trials
    [~, ~, ~, rs, ~, ~, ~] = LIF_network_fnc(file_path, scaling_factor,...
        u, stims, down_sample, use_initial_weights);
    pcs = pca(rs);
    pcs_sameneg = pcs_sameneg + pcs(:,1:3);
    clear pcs
end
pcs_sameneg = pcs_sameneg / n_trials;


% -1/+1
pcs_diffneg = zeros(T*100, 3);
u = zeros(2, T+1); % input stim
u(1, stim_on:stim_on+stim_dur) = -1; % first stim is -1
u(2, stim_on+stim_dur+delay:stim_on+2*stim_dur+delay) = -1; % second stim is -1
for i=1:n_trials
    [~, ~, ~, rs, ~, ~, ~] = LIF_network_fnc(file_path, scaling_factor,...
        u, stims, down_sample, use_initial_weights);
    pcs = pca(rs);
    pcs_diffneg = pcs_diffneg + pcs(:,1:3);
    clear pcs
end
pcs_diffneg = pcs_diffneg / n_trials;
...
h = plot3(pcs_samepos(stim1_time-3000:end,1)', pcs_samepos(stim1_time-3000:end,2)', pcs_samepos(stim1_time-3000:end,3)','r'); 
set(h,{'DisplayName'},{'+1/+1'})
scatter3(pcs_samepos(stim1_time,1), pcs_samepos(stim1_time,2), pcs_samepos(stim1_time,3), 100, 'filled', 'r','<')
scatter3(pcs_samepos(stim2_time,1), pcs_samepos(stim2_time,2), pcs_samepos(stim2_time,3), 100, 'filled', 'r')
scatter3(pcs_samepos(end,1), pcs_samepos(end,2), pcs_samepos(end,3), 300, 'r','filled','pentagram')

h = plot3(pcs_sameneg(stim1_time-3000:end,1)', pcs_sameneg(stim1_time-3000:end,2)', pcs_sameneg(stim1_time-3000:end,3)','blue'); 
set(h,{'DisplayName'},{'-1/-1'})
scatter3(pcs_sameneg(stim1_time,1), pcs_sameneg(stim1_time,2), pcs_sameneg(stim1_time,3), 100, 'b', 'filled', '<')
scatter3(pcs_sameneg(stim2_time,1), pcs_sameneg(stim2_time,2), pcs_sameneg(stim2_time,3), 100, 'b', 'filled')
scatter3(pcs_sameneg(end,1), pcs_sameneg(end,2), pcs_sameneg(end,3), 300, 'blue','filled','pentagram')

h = plot3(pcs_diffpos(stim1_time-3000:end,1)', pcs_diffpos(stim1_time-3000:end,2)', pcs_diffpos(stim1_time-3000:end,3)','magenta');
set(h,{'DisplayName'},{'+1/-1'})
scatter3(pcs_diffpos(stim1_time,1), pcs_diffpos(stim1_time,2), pcs_diffpos(stim1_time,3), 100, 'filled', 'magenta','<')
scatter3(pcs_diffpos(stim2_time,1), pcs_diffpos(stim2_time,2), pcs_diffpos(stim2_time,3), 100, 'filled', 'magenta')
scatter3(pcs_diffpos(end,1), pcs_diffpos(end,2), pcs_diffpos(end,3), 300, 'magenta','filled','pentagram')

h = plot3(pcs_diffneg(stim1_time-3000:end,1)', pcs_diffneg(stim1_time-3000:end,2)', pcs_diffneg(stim1_time-3000:end,3)','g');
set(h,{'DisplayName'},{'-1/+1'})
scatter3(pcs_diffneg(stim1_time,1), pcs_diffneg(stim1_time,2), pcs_diffneg(stim1_time,3), 100, 'filled', 'g','<')
scatter3(pcs_diffneg(stim2_time,1), pcs_diffneg(stim2_time,2), pcs_diffneg(stim2_time,3), 100, 'filled', 'g')
...
% downsampled figure
ds = 100;
pcs_samepos_ds = downsample(pcs_samepos, ds);
pcs_sameneg_ds = downsample(pcs_sameneg, ds);
pcs_diffpos_ds = downsample(pcs_diffpos, ds);
pcs_diffneg_ds = downsample(pcs_diffneg, ds);

figure; hold on; view(3);
h = plot3(pcs_samepos_ds(10:end,1)', pcs_samepos_ds(10:end,2)', pcs_samepos_ds(10:end,3)','r'); 
set(h,{'DisplayName'},{'+1/+1'})
h = plot3(pcs_sameneg_ds(10:end,1)', pcs_sameneg_ds(10:end,2)', pcs_sameneg_ds(10:end,3)','blue'); 
set(h,{'DisplayName'},{'-1/-1'})
h = plot3(pcs_diffpos_ds(10:end,1)', pcs_diffpos_ds(10:end,2)', pcs_diffpos_ds(10:end,3)','magenta');
set(h,{'DisplayName'},{'+1/-1'})
h = plot3(pcs_diffneg_ds(10:end,1)', pcs_diffneg_ds(10:end,2)', pcs_diffneg_ds(10:end,3)','g');
set(h,{'DisplayName'},{'-1/+1'})
legend show
scatter3(pcs_samepos_ds(stim1_time/ds,1), pcs_samepos_ds(stim1_time/ds,2), pcs_samepos_ds(stim1_time/ds,3), 100, 'filled', 'r','<')
scatter3(pcs_samepos_ds(stim2_time/ds,1), pcs_samepos_ds(stim2_time/ds,2), pcs_samepos_ds(stim2_time/ds,3), 100, 'filled', 'r')
scatter3(pcs_samepos_ds(end,1), pcs_samepos_ds(end,2), pcs_samepos_ds(end,3), 300, 'r','filled','pentagram')
scatter3(pcs_sameneg_ds(stim1_time/ds,1), pcs_sameneg_ds(stim1_time/ds,2), pcs_sameneg_ds(stim1_time/ds,3), 100, 'filled', 'b','<')
scatter3(pcs_sameneg_ds(stim2_time/ds,1), pcs_sameneg_ds(stim2_time/ds,2), pcs_sameneg_ds(stim2_time/ds,3), 100, 'filled', 'b')
scatter3(pcs_sameneg_ds(end,1), pcs_sameneg_ds(end,2), pcs_sameneg_ds(end,3), 300, 'b','filled','pentagram')
scatter3(pcs_diffpos_ds(stim1_time/ds,1), pcs_diffpos_ds(stim1_time/ds,2), pcs_diffpos_ds(stim1_time/ds,3), 100, 'filled', 'magenta','<')
scatter3(pcs_diffpos_ds(stim2_time/ds,1), pcs_diffpos_ds(stim2_time/ds,2), pcs_diffpos_ds(stim2_time/ds,3), 100, 'filled', 'magenta')
scatter3(pcs_diffpos_ds(end,1), pcs_diffpos_ds(end,2), pcs_diffpos_ds(end,3), 300, 'magenta','filled','pentagram')
