...
function [W, REC, spk, rs, all_fr, out, params] = LIF_network_lesion_neurons_fnc(model_path,...
scaling_factor, u, stims, downsample, neurons_idx, lesion_direction)
...
%   - neurons_idx: the indices of the neurons that should be lesioned
%   - lesion_direction: "presynaptic" or "postsynaptic", indicating which
%   direction lesioning should occur
%
% OUTPUT
%   - W: recurrent connectivity matrix scaled by the scaling factor (N x N)
%   - REC: membrane voltage from all the units (N x t)
%   - spk: binary matrix indicating spikes (N x t)w
...
load(model_path, 'w_in', 'w', 'N', 'm', 'som_m', 'w_out', ...
...
if strcmpi(lesion_direction, "presynaptic") == 1 % Inh -> Inh
    W(neurons_idx , : ) = W(neurons_idx , : )*0.5;
elseif strcmpi(lesion_direction, "postsynaptic") == 1 % Exc -> Inh
    W(: , neurons_idx) = W(: , neurons_idx)*0.5;
else
    disp('not a proper lesioning direction, proceeding without lesioning')
end
