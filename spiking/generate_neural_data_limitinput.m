...
for n_model = 1:length(model_list)
    ...
    neural_save_name = [this_model_dir,'/','neuraldata',norm_name,lesion_name,'_delay',num2str(delay),input_to,'only','.mat'];
    bhv_save_name = [this_model_dir,'/','bhvdata',norm_name,lesion_name,'_delay',num2str(delay),input_to,'only','.mat'];
    timing_save_name = [this_model_dir,'/','timingdata',norm_name,lesion_name,'_delay',num2str(delay),input_to,'only','.mat'];
    ...
    for ii = stim1s
        ...
        for jj = stim2s
            ...
            parfor n_trial=1:n_trials_per_condn
                [~, ~, spk_train, rates, ~, outputs, params] = LIF_network_fnc_limitinput(model_path, scaling_factor,...
                    u, stims, lfp_input, down_sample, use_initial_weights, input_to);
