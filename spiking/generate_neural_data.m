...
cd(all_model_path)
% model_list = dir('*.mat');
model_list = dir('*Jitter_10_2*.mat');
...
fs_out = 100; % downsample frequency for outputs
...
for n_model = 1:length(model_list)
    ...
    disp(['max stable performance ', num2str(max(all_perfs))])

    % skip models with performance <95%
    ...
    all_trial_outputs = zeros(n_trials_total, ceil(T/fs_rate*fs_out));
